...
{ 
  if(empty() || elt < head->data){
	head=new node<eltType>(elt,head);
	return NULL;}
...
/*  else if(elt < head->data){
	...
	if(p->data==elt)
		...
		trailp->next = new node<eltType>(elt,p);
}		
}
...
// *** Make it match the prerequisite that the item will be found (check)******
